...
///	unlink(SP_MON_DAEMON_FIFO1);

	if (mkfifo(SP_MON_DAEMON_FIFO1 , 0777) < 0) {
#if 0
        perror("Failed::");
	...
	fd = open(SP_MON_DAEMON_FIFO1, O_RDONLY);
	if (fd < 0)
		printf("Failed to open fifo"SP_MON_DAEMON_FIFO1"\n");
    ...
    if (ret > 0) {
        ...
        buf[0] = SP_MON_DAEMON_FIFO1_INDEX;
