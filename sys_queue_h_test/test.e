...
# 1 "/usr/lib/gcc/i686-redhat-linux/4.4.7/include/stddef.h" 1 3 4
# 211 "/usr/lib/gcc/i686-redhat-linux/4.4.7/include/stddef.h" 3 4
...
struct file_storage{
    ...
    struct { struct file_storage *le_next; struct file_storage **le_prev; } entry;
};

struct storage_head { struct file_storage *lh_first; } *gStorage;
...
int file_storage_register(const char *pathname, pthread_mutex_t *mutex)
{
    ...
    do { if (((fs)->entry.le_next = (gStorage)->lh_first) != ((void *)0)) (gStorage)->lh_first->entry.le_prev = &(fs)->entry.le_next; (gStorage)->lh_first = (fs); (fs)->entry.le_prev = &(gStorage)->lh_first; } while ( 0);
}
...
void file_save(const char *filename)
{
    if (access(filename, 0) == 0) {
        ...
        printf("mv %s.tar.gz /storage/ ", filename, filename);
    }

}
void *file_storage_task(void *arg)
{
    struct file_storeage *fs;
    gStorage = (struct storage_head *) malloc(struct storage_head));
    ...
    while(1) {
        ...
        for ((fs) = ((gStorage)->lh_first); (fs); (fs) = ((fs)->entry.le_next)) {
...
int file_storage_init(const char *dst_dir)
{
    ...
    if (pthread_create(&tid_sys_utilz, ((void *)0), (FUNCPTR)file_storage_task, ((void *)0)) != 0) {
