...
int main(int argc, char *argv[])
{
	...
	/* create a document */
	memset(&document, 0x00, sizeof(document));
	version_directive.major = 1;
	version_directive.minor = 1;
	if (!yaml_document_initialize(&document, &version_directive, NULL, NULL, 0, 0)) {
		printf("Failed to initialize one document!\n");
		return -1;
	}
	
    properties = yaml_document_add_mapping(&document, NULL, YAML_BLOCK_MAPPING_STYLE);
//    yaml_document_append_sequence_item(&document, -1, properties);
