...
#define CLIENT_DEBUG(FMAT,ARGS...) printf("[client]%s %d:"FMAT"\n", __func__, __LINE__, ## ARGS)
...
int main(int argc, char **argv)  
{
    ...
    if ((skfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {  
        CLIENT_DEBUG("create socket failed");
        exit(1);  
    }
    ...
    if((ret = connect(skfd, (struct sockaddr *)&server_addr, sizeof(server_addr))) < 0) { 
        CLIENT_DEBUG("connect failed, errno = %d, ret = %d", errno, ret);
		perror("error :");
		ts2 = time(NULL);
		CLIENT_DEBUG("time interval is ts2 - ts1 = %d", ts2 - ts1);
        exit(1);
    }
 
    CLIENT_DEBUG("connect successfully!\n");

#if 1
    int cnt = 0;
    while (cnt--) {
        char a = 0x11;
        retlen = write(skfd, recvbuf, 10);
        CLIENT_DEBUG("retlen = %d, sizeof(recvBuf) = %d", retlen, sizeof(recvbuf));
    }
    sleep(10);
    //close(skfd);
    return 0;
